...
// Solution code here...
let long = [...arr];
return arr.indexOf(long.sort((a, b) => b.length - a.length)[0]);
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map((ele) =>ele.split("(")[1].split(") ").join("").split("-").join(""))
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split("").filter((ele, index) => index % 2 !== 0).join("");
};
