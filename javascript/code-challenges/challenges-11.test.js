...
const count = (target, input) => input.map((arr) =>arr.filter((num) => num === target)).reduce((a, newarr) => a + newarr.length, 0);
...
const totalSum = (input) =>input.map(arr=>arr.reduce((a,num)=>a+num,0)).reduce((a,sum)=>a+sum,0);
  // Solution code here...
...
let findMaleAndFemale = (data) =>{ 
  ...
  let arr=data.filter((obj) => obj.gender === "female" ||obj.gender === "male");
  return arr.reduce((a, obj, index) => a +(index ? "and " : "") + obj.name + (index < arr.length-1? " " : "") , "");
...
let findShortest = (data) =>  data.sort((a, b) => Number(a.height) - Number(b.height))[0].name;
  // Solution code here...
